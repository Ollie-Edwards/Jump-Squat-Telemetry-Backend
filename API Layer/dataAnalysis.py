...
def calibrate_if_stationary(a_inertial, sampling_rate):
    ...
    if np.all(
        (a_inertial[:n_samples] - a_inertial[0] >= -0.3)
        & (a_inertial[:n_samples] - a_inertial[0] <= 0.3)
    ):
...
def analyseDataframe(df):
    ...
    if (
        (start_idx == None)
        or (takeoff_idx == None)
        or (takeoff_idx == start_idx)
        or (start_idx > takeoff_idx)
    ):
        return {"acceleration": list(a_inertial)[0::10], "time": list(t)[0::10]}
    ...
    v[start_idx:] = cumulative_trapezoid(
        a_inertial[start_idx:], t[start_idx:], initial=0
    )
    ...
    power = GRF * v
    plt.plot(
        t[start_idx:takeoff_idx],
        power[start_idx:takeoff_idx] / 1000,
        label="Pwr (Kw)",
        linewidth=2,
    )
    ...
    avg_power = (
        np.trapezoid(power[start_idx:takeoff_idx], t[start_idx:takeoff_idx])
        / time_window
    )
